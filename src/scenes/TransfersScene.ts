...
export class TransfersScene extends Phaser.Scene {
  ...
  create(data: { dynasty: DynastyState; note?: string }): void {
    ...
    all.forEach((r, i) => {
      ...
      scroll.add(this.add.rectangle(width / 2, y + 30, width - 24, rowH - 8, COLORS.panel, 1).setStrokeStyle(1, COLORS.stroke));
      ...
      scroll.add(this.add.text(42, y + 31, `${riderType(r)} · ${r.nationality} · age ${r.age} · now ${riderRating(r)}`, { fontFamily: FONT, fontSize: '10px', color: COLORS.textMuted }).setOrigin(0, 0.5));
      // current stats — viewable before you commit the fee
      scroll.add(this.add.text(42, y + 50, statLine(r), { fontFamily: FONT, fontSize: '10px', color: '#8fb4c8' }).setOrigin(0, 0.5));
      // potential (fuzzy for the young → gold to flag the gamble)
      scroll.add(this.add.text(width - 92, y + 31, `${stars} ${young ? sc.label : ''}`.trim(), { fontFamily: FONT, fontSize: '10px', color: young ? '#f5c518' : COLORS.textMuted }).setOrigin(1, 0.5));
      scroll.add(this.add.text(width - 92, y + 12, `fee ${signingFeeFor(riderRating(r))} · ${salaryOf(r)}/yr`, { fontFamily: FONT, fontSize: '11px', color: '#f5c518' }).setOrigin(1, 0.5));
      const signButton = makeButton(this, width - 48, y + 30, 'Sign', () => this.sign(r.id), { width: 74, height: 26, fontSize: 12, fill: COLORS.buttonSelected });
      scroll.add(signButton.container);
    });
  }
