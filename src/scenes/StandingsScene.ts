...
export class StandingsScene extends Phaser.Scene {
  ...
  create(data: { dynasty: DynastyState }): void {
    ...
    if (this.dynasty.world) {
      this.riderBtn = makeButton(this, 70, 74, 'Riders', () => this.setMode('riders'), { width: 104, height: 34, fontSize: 13 });
      this.teamBtn = makeButton(this, width / 2, 74, 'Pro Tour', () => this.setMode('pro'), { width: 104, height: 34, fontSize: 13 });
      this.worldBtn = makeButton(this, width - 70, 74, 'World', () => this.setMode('world'), { width: 104, height: 34, fontSize: 13 });
    } else {
  ...
  private setMode(mode: 'riders' | 'teams' | 'pro' | 'world'): void {
    this.mode = mode;
    this.render();
  }
  ...
  private render(): void {
    ...
    this.teamBtn.setSelected(this.mode === (this.dynasty.world ? 'pro' : 'teams'));
    this.worldBtn?.setSelected(this.mode === 'world');
    ...
    const rows = this.mode === 'riders'
      ? riderStandings(season).slice(0, 22).map((r) => ({ id: r.id, name: byId.get(r.id)!.name, teamId: byId.get(r.id)!.teamId, points: r.points }))
      : this.dynasty.world && (this.mode === 'pro' || this.mode === 'world')
        ? rankDivision(this.dynasty.world, this.mode, this.dynasty.seasonNumber)
          .map((row) => ({ id: row.teamId, name: dynastyTeamName(this.dynasty, row.teamId), teamId: row.teamId, points: row.points }))
        : teamStandings(season, (id) => teamOf(this.dynasty, id))
          .map((t) => ({ id: t.id, name: dynastyTeamName(this.dynasty, t.id), teamId: t.id, points: t.points }));
    ...
    rows.forEach((row, i) => {
      ...
      const movementZone = this.mode === 'pro' ? i < 2 : this.mode === 'world' ? i >= rows.length - 2 : false;
      if (isPlayer) this.listItems.push(this.add.rectangle(width / 2, y, width - 22, rowH - 4, COLORS.buttonSelected, 0.12));
      if (movementZone) this.listItems.push(this.add.rectangle(width / 2, y, width - 22, rowH - 4, this.mode === 'pro' ? COLORS.positive : COLORS.coral, 0.08));
      ...
      if (this.mode === 'pro' && i === 1 || this.mode === 'world' && i === rows.length - 3) {
        const label = this.mode === 'pro' ? 'PROMOTION LINE' : 'RELEGATION LINE';
        this.listItems.push(this.add.text(width - 24, y + rowH / 2 - 1, label, { fontFamily: FONT, fontSize: '8px', color: COLORS.textMuted }).setOrigin(1, 0.5));
      }
    });
  }
}
